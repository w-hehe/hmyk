define(['jquery', 'bootstrap', 'backend', 'table', 'form'], function ($, undefined, Backend, Table, Form) {
    ...
    var Controller = {
        index: function () {
            ...
            //添加向发件人发送测试邮件按钮和方法
            $('input[name="row[mail_from]"]').parent().next().append('<a class="btn btn-info testmail" style="margin-top: 10px; margin-bottom: 6px;">发送测试邮件</a>');
            $(document).on("click", ".testmail", function () {
                var that = this;
                layer.confirm('邮件将发送到管理员邮箱', {
                    title:'温馨提示',
                    btn: ['立即发送','取消'] //按钮
                }, function(index){
                    layer.close(index);
                ...
                Layer.confirm({title: '请输入接收邮箱', formType: 0}, function (value, index) {
                    Backend.api.ajax({
                        url: "general/config/emailtest",
